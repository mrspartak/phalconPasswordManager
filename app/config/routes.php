...
$app->post(
    ...
    function () use ($app) {
        ...
        $data['text_secret_key'] = Users::sha1Rounds(
                                        $app->config->app->static_salt . $text_secret_key . $data['salt']
        );
        ...
        switch ($action) {
            case 'get_secret':
                ...
                if (Users::sha1Rounds($app->config->app->static_salt . $data['secret_key'] . $row->salt)
                    ...
                    {
                        $app->response->setStatusCode(500, "Wrong credentials")->sendHeaders();
                        exit();
                    }
                }
                break;
